...
class Pix2PixModel(BaseModel):
    ...
    def backward_D(self):
        ...
        if self.opt.L1_mode == 'contrastive':
            contrastive_fake = self.netC(fake_AB.detach())
            self.loss_C_fake = self.criterionContrastiveL1( contrastive_fake, False )
        ...
        if self.opt.L1_mode == 'contrastive':
            contrastive_real = self.netC(real_AB)
            self.loss_C_real = self.criterionContrastiveL1( contrastive_real, True )
    ...
    def backward_G(self):
        ...
        # Second, G(A) = B
        if self.opt.L1_mode == 'contrastive':
            contrastive_fake = self.netC(fake_AB)
            self.loss_G_L1 = self.criterionContrastiveL1( contrastive_fake, True ) * self.opt.lambda_L1
        else:
