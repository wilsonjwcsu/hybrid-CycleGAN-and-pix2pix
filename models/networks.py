...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    elif netG == 'resnet_6blocks_sn':
        net = ResnetGenerator_SN(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=6)
    elif netG == 'unet_128':
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netD == 'basic':  # default PatchGAN classifier
        ...
        net = NLayerDiscriminator_SN(input_nc, ndf, n_layers_D)
...
class GANLoss(nn.Module):
    ...
    def __init__(self, gan_mode, target_real_label=1.0, target_fake_label=0.0):
        ...
        elif gan_mode in ['wgangp', 'wgansn']:
            self.loss = None
        elif gan_mode == 'contrastive':
            self.loss = nn.L1Loss()
            self.m = 1.0
        else:
    ...
    def __call__(self, prediction, target_is_real):
        ...
        elif self.gan_mode in ['wgangp', 'wgansn']:
            if target_is_real:
                loss = -prediction.mean()
            else:
                loss = prediction.mean()
        elif self.gan_mode == 'contrastive':
            if target_is_real:
                loss = self.loss(prediction[:,0], prediction[:,1])
            else:
                #loss = 1.0 - self.loss(prediction[:,0], prediction[:,1])
                loss = torch.nn.functional.relu(self.m - self.loss(prediction[:,0], prediction[:,1]))
...
class ResnetGenerator_SN(nn.Module):
    """Resnet-based generator that consists of Resnet blocks between a few downsampling/upsampling operations. This one adds spectral normalization.
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        model = [nn.ReflectionPad2d(3),
                 spectral_norm(nn.Conv2d(input_nc, ngf, kernel_size=7, padding=0, bias=use_bias)),
                 norm_layer(ngf),
                 nn.ReLU(True)]
        ...
        for i in range(n_downsampling):  # add downsampling layers
            mult = 2 ** i
            model += [spectral_norm(nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias)),
                      norm_layer(ngf * mult * 2),
                      nn.ReLU(True)]
...
class ResnetBlock_SN(nn.Module):
    """Define a Resnet block with spectral normalization"""
...
class NLayerDiscriminator_SN(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3):
        ...
        print("spectral norm discriminator")
...
class ContrastiveDiscriminator(nn.Module):
    """Defines a convolutional encoder"""
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d):
        ...
        self.nc = input_nc
        self.enc = Encoder( input_nc, ndf, n_layers, norm_layer )

    def forward(self, input):
        """Standard forward."""
        z1 = self.enc(input[:,0:1,:,:])
        z2 = self.enc(input[:,1:2,:,:])
        z = torch.cat((z1,z2))
        return z
